...
namespace BugTracker.API.Controllers
{
    ...
    public class BugController : ControllerBase
    {
        ...
        // PATCH /api/bug/{id}/assign - Only Admin can assign bugs
        [HttpPatch("{id}/assign")]
        [Authorize(Roles = "Admin")]
        public async Task<ActionResult> AssignBugAsync(int id, [FromBody] AssignBugRequestDTO dto)
        {
            var existing = await _service.GetBugByIdAsync(id);
            if (existing == null)
                return NotFound();

            await _service.AssignBugAsync(id, dto.AssignedToUserId);
            return Ok(new { Message = "Bug assigned successfully." });
        }

        // PATCH /api/bug/{id}/status - Testers can update bug status
        [HttpPatch("{id}/status")]
        [Authorize(Roles = "Admin,Tester")]
        public async Task<ActionResult> UpdateBugStatusAsync(int id, [FromBody] UpdateBugStatusDTO dto)
        {
            var existing = await _service.GetBugByIdAsync(id);
            if (existing == null)
                return NotFound();

            await _service.UpdateBugStatusAsync(id, dto.Status);
