...
@Component({
  selector: 'app-bugs',
  standalone: true,
  imports: [CommonModule, CardModule, ButtonModule, FormsModule, DropdownModule, InputTextModule, TableModule, TagModule, DialogModule, InputTextareaModule],
...
export class BugsComponent implements OnInit {
 ...
 ngOnInit(): void {
  ...
  isTester(): boolean {
    return this.authService.getUserRole() === 'Tester';
  }
  ...
  updateStatus(bugId: number): void {
    this.selectedBugId = bugId;
    const bug = this.bugs.find(b => b.id === bugId);
    this.selectedStatus = bug?.status || 'Open';
    this.showStatusDialog = true;
  }

  onUpdateStatus(): void {
    if (this.selectedBugId && this.selectedStatus) {
      this.bugService.updateBugStatus(this.selectedBugId, this.selectedStatus).subscribe({
        next: () => {
        ...
        error: (err) => {
          ...
          alert('Failed to update bug status: ' + (err.error?.message || err.message || 'Unknown error'));
        }
      });
    }
  }
  ...
  viewBugDetail(bugId: number): void {
    this.router.navigate(['/bug-detail', bugId]);
  }
}
